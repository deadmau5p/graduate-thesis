...
class StanceClassification:
    ...
    def equally_distribute(self):
        ...
        self.target_dataset.df_train = util.shuffle_df(all_df)
        logger.debug(self.target_dataset.df_train)
        ...
        t_f = x[x['Stance'] == 'FAVOR'].sample(n, random_state=42)
        all_df = pd.concat([t_a, pd.concat([t_f, t_n], ignore_index=True)], ignore_index=True)
        self.target_dataset.df_test = util.shuffle_df(all_df)
        ...
        for lr in [2e-5, 3e-5, 5e-5]:
            for epochs in [5, 6, 7, 8]:
                for lang, model in [('slovenian', "EMBEDDIA/sloberta")]:
                    if target != "Climate Change is a Real Concern":
                        train_model(lr, epochs, target, lang, model, [target], [target])
                    # train_model(lr, epochs, target, lang, model, [target], [target], add_target=False)
    for lr in [2e-5, 3e-5, 5e-5]:
        for epochs in [5, 6, 7, 8]:
            for lang, model in [('slovenian', "EMBEDDIA/sloberta")]:
                train_model(lr, epochs, 'Janez Janša', lang, model, [target], [target], add_target=False)
...
if __name__ == "__main__":
    ...
    train_model(5e-5, 3, 'all', 'english', "EMBEDDIA/crosloengual-bert",
                all_targets, all, add_target=True)
    train_model(5e-5, 2, 'all', 'slovenian', "EMBEDDIA/crosloengual-bert",
                all_targets, all, add_target=True)
    train_model(5e-5, 3, 'all', 'slovenian', "EMBEDDIA/sloberta", all_targets,
                all, add_target=True)
