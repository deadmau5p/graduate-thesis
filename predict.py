...
def predict_on_each_sentence(target, vsebina, phrases_text, model, tokenizer, language_b, all):
    ...
    for stavek in util.split_into_sentences(vsebina, phrases_text):
        ...
        # logger.info(f'End: {stavek} {stance}, {probs}')
        new.append(list(probs))
    if not new:
        return 'NONE', None
    probs = util.aggregate(new)
    # logger.info(f'End: {vsebina} {util.id2label[np.argmax(probs)]}, {probs}')
...
if __name__ == "__main__":
    ...
    # tokenizer_ = AutoTokenizer.from_pretrained('./models/all_l_5e-05_slovenian_3_True')
    # model_ = AutoModelForSequenceClassification.from_pretrained('./models/all_l_5e-05_slovenian_3_True',
    #                                                             num_labels=3)
    # compute_f1(target="Atheism", phrases_text=util.ateizem_lemma_text, phrases=util.ateizem_lemma,
    ...
    # #
    # tokenizer_ = AutoTokenizer.from_pretrained('./models/all_a_5e-05_slovenian_2_True')
    # model_ = AutoModelForSequenceClassification.from_pretrained('./models/all_a_5e-05_slovenian_2_True',
    #                                                             num_labels=3)
    # compute_f1(target="Atheism", phrases_text=util.ateizem_lemma_text, phrases=util.ateizem_lemma,
    #            prefix='atheism', cel_doc=False, model=model_, tokenizer=tokenizer_, language_b=False, all_=False)
    #
    ...
    #            prefix='atheism', cel_doc=True, model=model_, tokenizer=tokenizer_, language_b=False, all_=False)
    #
    # tokenizer_ = AutoTokenizer.from_pretrained('./models/all_a_5e-05_english_3_True')
    # model_ = AutoModelForSequenceClassification.from_pretrained('./models/all_a_5e-05_english_3_True',
    #                                                             num_labels=3)
    # compute_f1(target="Atheism", phrases_text=util.ateizem_lemma_text, phrases=util.ateizem_lemma,
    ...
    #     './models/all_a_5e-05_english_3_True',
    #     num_labels=3)
    # compute_f1(target="Legalization of Abortion", phrases_text=util.splav_lemma_text,
    ...
    # model_ = AutoModelForSequenceClassification.from_pretrained('./models/all_a_5e-05_english_3_True',
    #                                                             num_labels=3)
    # compute_f1(target="Janez Janša", phrases_text=util.feminizem_lemma_text, phrases=util.feminizem_lemma,
